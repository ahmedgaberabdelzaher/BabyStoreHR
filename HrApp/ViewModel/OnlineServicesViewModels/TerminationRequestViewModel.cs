...
namespace HrApp.ViewModel.OnlineServicesViewModels
{
    ...
    public class TerminationRequestViewModel : DetailsandActionsViewModel<TerminationRequest>
    {
    ...
    CustodyProperityLst selectedReason;
        ...
        public TerminationRequestViewModel(Services.Interface.IOnlineServices onlineServices, INavigationService navigationService, IPageDialogService pageDialogService, Services.Interface.IUserServices userServices) : base(onlineServices,navigationService, pageDialogService, userServices)
        {
        ...
        public DelegateCommand SelectReasonCommand
        {
            get
            {
                ...
                {
                    IsLoading = true;
                    var Data = await GetReasons();
                    if (Data != null)
                    {
                        var rs = await DialogService.DisplayActionSheetAsync("Select Reason", "Cancel", "", Data.Select(c => c.name).ToArray());
                        if (rs != null && rs != "Cancel")
                        {
                            Reason = rs;
        ...
        async Task SendRequest()
        {
            try
            {
                ...
                {
                    var model = new TerminationRequestBody()
                    {

                        employee_id = SelectedEmployee.id,
                        termination_reason_id = SelectedReason.id,
                        the_last_working_day = lastWorkingDay.ToString("yyyy-MM-dd")


                    };
                    var body = new BaseOdoModel<TerminationRequestBody>()
                    {
                        @params = model
                    };
                    var resp = await _onlineServices.SendRequest<TerminationRequestBody>(body, "termination/create_request");
        ...
        async Task GetAllRequests()
        {
            try
            {
                ...
                var resp = await _onlineServices.GetRequests<TerminationRequest>(model, "termination");
                if (resp.Item2)
                {
        ...
        public void SetDetailsData(TerminationRequest data)
        {
            ...
            Reason = data.termination_reason_id[1].ToString();
            SelectedReason = new CustodyProperityLst() { display_name = data.termination_reason_id[1].ToString(), name = data.termination_reason_id[1].ToString() };
            LaymenlastWorkingDaytDate = DateTimeHelper.ConvertStringTodate(data.the_last_working_day);
